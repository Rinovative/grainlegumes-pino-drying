...
CASE_WORKSPACE_MARKER: Final = ".generation-case-workspace.json"
...
def _require_user_owned_writable(path: Path, *, label: str) -> None:
...
def resolve_storage_root(
    ...
    *,
    ...
    resolved = _absolute_path(configured, label="storage_root")
    home = Path.home().resolve()
    repository = common.paths.get_project_root().resolve()
    if resolved in {Path("/"), home, repository} or _is_relative_to(resolved, repository):
        message = f"storage_root targets a forbidden persistent boundary: {resolved}"
        raise ValueError(message)
    _require_user_owned_writable(resolved, label="storage_root")
    if create:
...
def resolve_work_root(
    *,
    ...
    if work_root is None:
        ...
        resolved = _absolute_path(configured, label="TMPDIR scratch root")
        if not resolved.is_dir() or not os.access(resolved, os.W_OK | os.X_OK):
            message = f"TMPDIR scratch root is unavailable: {resolved}"
            raise PermissionError(message)
    else:
        resolved = _absolute_path(work_root, label="work_root")
        _require_user_owned_writable(resolved, label="work_root")
        if create:
            resolved.mkdir(parents=True, exist_ok=True)
        _require_user_owned_writable(resolved, label="work_root")
    repository = common.paths.get_project_root().resolve()
    ...
    if resolved in forbidden or _is_relative_to(resolved, repository) or _is_relative_to(resolved, storage_root):
...
def create_case_workspace(
    ...
    *,
    ...
    if not _is_relative_to(directory, root) or directory.parent != root:
...
def _guard_cleanup_target(
    ...
    *,
    ...
    resolved_target = _absolute_path(target, label="cleanup target")
    resolved_root = _absolute_path(allowed_root, label="allowed cleanup root")
    if not resolved_target.exists() or not resolved_target.is_dir() or resolved_target.is_symlink():
        ...
        raise ValueError(message)
    if resolved_target == resolved_root or not _is_relative_to(resolved_target, resolved_root):
    ...
    if resolved_target in forbidden or any(
        _is_relative_to(forbidden_path, resolved_target) for forbidden_path in forbidden if forbidden_path != Path("/")
    ):
...
def initialize_worker_workspace(
    ...
    *,
    ...
    resolved = _absolute_path(directory, label="worker workspace")
    storage = resolve_storage_root(storage_root, create=False)
...
def create_publication_staging(
    *,
    ...
    root = _absolute_path(publication_root, label="publication staging root")
    if not _is_relative_to(root, storage):
...
def validate_transfer_staging(
    ...
    *,
    ...
    resolved = _absolute_path(directory, label="transfer staging")
    marker = _load_marker(
