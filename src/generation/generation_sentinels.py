...
Design principles:
  ...
  - A diagnostic retry may expose downstream effects but cannot clear a guard failure
...
_STATIC_SENTINEL_SCHEMA_VERSION: Final = 2
...
def _diagnostic_fields(
    ...
    *,
    ...
    """Generate one field realization with an explicit diagnostic support policy."""
    ...
    return field_service.generate_spatial_fields(
        ...
        porosity_support_departure_authorization=porosity_support_departure_authorization,
    )
...
def _realization_evidence(
    ...
    active_unit = sample.ood_provenance["active_unit_id"]
    porosity_ood_active = active_unit in materials.POROSITY_GENERATOR_PARAMETERS
    ...
    try:
        fields = _diagnostic_fields(
            ...
            case_index,
            sample,
            porosity_support_departure_authorization=("active_porosity_parameter_ood" if porosity_ood_active else None),
        )
    except ValueError as error:
        if porosity_ood_active or not str(error).startswith("Generated mean porosity violates the material natural support"):
            raise
        guard_failure = str(error).split(", diagnostics=", maxsplit=1)[0]
        fields = _diagnostic_fields(
            batch,
            case_index,
            sample,
            porosity_support_departure_authorization="static_sentinel_diagnostic_bypass",
        )
    ...
    result: dict[str, Any] = {
        ...
        "porosity_guard_failure": guard_failure,
        "diagnostic_porosity_support_bypass_used": guard_failure is not None,
    }
